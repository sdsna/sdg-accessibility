...
localDir = "/Users/guilhermeiablonovski/Dropbox (SDSN)/SDG Geospatial Indicators Project/sdg-accessibility"
popDir = "data/processed/UApolys/"+continent
accessDir = "data/processed/access-"+continent
gridAccessDir = "data/processed/gridAccess/"+continent
aggDir = "data/processed/agg"


# urban centers database for later
uc = pd.read_csv(f"{localDir}/data/GHS_STAT_UCDB2015MT_GLOBE_R2019A/GHS_STAT_UCDB2015MT_GLOBE_R2019A_V1_2.csv", encoding = "ISO-8859-1", engine='python')
...
popData = [file.split(".")[0] for file in os.listdir(popDir) if file.split(".")[1] == "gpkg"]
...
for UA in tqdm(r_cities):
    ...
    uaAccess = pd.read_csv(f"{accessDir}/{UA}.0.csv")#[["id",	"active_living",	"community_space",	"education",	"food_choices",	"health_wellbeing",	"mobility",	"nightlife",	"x",	"y",	"ID_HDC_G0",	"CTR_MN_NM",	"UC_NM_MN",	"P15",	"AREA",	"UC_NM_CTR",	"geometry"]]
    uaPop = gpd.read_file(f"{popDir}/{UA}.gpkg")
